...
/*   Updated: 2021/01/20 00:05:02 by lsuardi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
_maxEnergyPoints(100), _level(1), _meleeAttackDmg(30), _rangedAttackDmg(20), _armorDmgReduction(5)
{
	...
	std::cout << '<' << name << "> - " << fct::randomString(JOIN_Q) << std::endl;
}
...
void	FragTrap::rangedAttack(std::string const &target)
{
	...
	else
	{
		...
		std::cout << '<' << _name << "> - " << fct::randomString(RGED_Q) << std::endl;
	}
}
...
void	FragTrap::meleeAttack(std::string const &target)
{
	...
	else
	{
		...
		std::cout << '<' << _name << "> - " << fct::randomString(MLEE_Q) << std::endl;
	}
}
...
void	FragTrap::takeDamage(unsigned int amount)
{
	...
	else
	{
		...
		std::cout << '<' << _name << "> - " << fct::randomString(HURT_Q) << std::endl;
		if (_hitPoints <= 0)
		{
			std::cout << '<' << _name << "> - " << fct::randomString(DEAD_Q) << std::endl;
...
void	FragTrap::beRepaired(unsigned int amount)
{
	...
	else
	{
		...
		std::cout << '<' << _name << "> - " << fct::randomString(REPA_Q) << std::endl;
	}
}
...
void	FragTrap::vaulthunter_dot_exe(std::string const &target)
{
	...
	else if (_energyPoints < 25)
	{
		...
		std::cout << '<' << _name << "> " << fct::randomString(OUTA_Q) << std::endl;
	}
	else
	{
		...
		std::cout << '<' << _name << "> - " << fct::randomString(VHEX_Q) << std::endl;
	}
}
...
std::string				fct::randomString(int i)
{
